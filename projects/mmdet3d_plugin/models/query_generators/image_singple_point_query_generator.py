...
class ImageSinglePointQueryGenerator(BaseModule):
    ...
    def center2lidar(self, center_pred, intrinsic, extrinsic):
        center_img = torch.cat([center_pred[:, :2] * center_pred[:, 2:3], center_pred[:, 2:3]], dim=1) # Chuyen toa do tu anh 2D sang 3D
        center_img_hom = torch.cat([center_img, center_img.new_ones([center_img.shape[0], 1])], dim=1)  # [num_rois, 4] --> Them 1 chieu thanh [x,y,d,1]
        lidar2img = torch.bmm(intrinsic.double(), extrinsic.double()) # ma tran chuyen toa do lidar ve toa do image
        img2lidar = torch.inverse(lidar2img).float() # ma tran chuyen toa do image sang lidar
        center_lidar = torch.bmm(img2lidar, center_img_hom[..., None])[:, :3, 0] # Nhan ma tran dd chuyen toa do image sang lidar
        return center_lidar
